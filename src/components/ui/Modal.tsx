    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn('relative bg-white rounded-xl shadow-xl w-full', sizes[size])}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E5E3]">
          ...
          <button onClick={onClose} className="text-[#6B6B6B] hover:text-[#130D10] p-1 rounded-lg hover:bg-[#F9F9F8]">
            <IconX size={16} />
          </button>
        </div>
        <div className="px-6 py-4 max-h-[70vh] overflow-y-auto">{children}</div>
        {footer && (
          <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#E5E5E3]">
            {footer}
          </div>
        )}
      </div>
